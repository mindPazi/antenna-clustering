...
def GenerateLattice(Ny, Nz, x1, x2, A=None, B=None):
    ...
    Optimized version with minor computational improvements
    ...
    # OPTIMIZATION 1: Set default values more efficiently
    if A is None:
        A = np.inf
        B = np.inf
    elif B is None:
    ...
    # OPTIMIZATION 2: Extract constants once
    dz = x2[1]
    dy = x1[0]
    DELTA = max(x2[0], x1[1])
    
    # OPTIMIZATION 3: Vectorized coordinate computation
    Y = NN * dy
    Z = MM * dz
    
    # Apply shift to alternating rows (vectorized operation)
    Y[1::2, :] = Y[1::2, :] + DELTA
    
    # OPTIMIZATION 4: Use ptp (peak-to-peak) instead of max - min
    DZ = np.ptp(Z)  # Equivalent to np.max(Z) - np.min(Z), but faster
    DY = np.ptp(Y)
    
    ### Aperture truncation
    
    # OPTIMIZATION 5: Vectorized truncation computation
    # Compute truncation condition in one operation
    truncation_condition = (Y/A)**2 + (Z/B)**2 > 1
    I = np.where(truncation_condition)
    
    # Create mask (vectorized)
    Mask = np.ones(Y.shape, dtype=float)
    Mask[I] = 0
