...
version.''} It lists four key optimizations: greedy initialization, local search
refinement, adaptive sampling with learned probabilities, and NumPy vectorized
kernel computation.
...
\begin{enumerate}
    ...
    \item \textbf{\texttt{\_generate\_polar\_coordinates}}: generates angular grids
          ...
          The spectral grid has $N_w = \lfloor 2 \cdot 4 D_{z,\text{tot}} / \lambda \rfloor$ and
          $N_v = \lfloor 2 \cdot 4 D_{y,\text{tot}} / \lambda \rfloor$ points,
          where $D_{z,\text{tot}} = D_z + d_z$ and $D_{y,\text{tot}} = D_y + d_y$
          include one extra element spacing beyond the aperture.
...
% ------------------------------------------------------------
\subsection{Case 0: $\theta=0^\circ$, $\phi=0^\circ$, CF$=2$, sizes $\{1,2,4\}$, SLL $< -20/-15\;\text{dB}$}
% ------------------------------------------------------------
...
SLL requirement & $< -20\;\text{dB}$ out-of-FoV, $< -15\;\text{dB}$ in-FoV (SLL\_level=20, SLLin=15) \\
\bottomrule
\end{tabular}
\end{center}
...
% ------------------------------------------------------------
\subsection{Case 3: $\theta=10^\circ$, $\phi=45^\circ$, CF$=2$, sizes $\{1,2,4\}$, SLL $< -20\;\text{dB}$}
% ------------------------------------------------------------
...
Cluster sizes & $\{1, 2, 4\}$ \\
SLL requirement & $< -20\;\text{dB}$ \\
CF target & $2$ (default, targeting $\sim$128 clusters) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{What happens.}
Diagonal steering with moderate compression (CF$=2$, targeting $\sim$128 clusters).
