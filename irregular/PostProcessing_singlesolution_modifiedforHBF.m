...
id_cluster=find(Yc_m>0);
%         id_cluster=find(Yc_m<0);
% Lsub=[ Lsub(id_cluster) Lsub(id_cluster)];

if flip_half_pannel==1
    Zc_m=[-Zc_m(id_cluster)+x2(2) Zc_m(id_cluster)];
    Yc_m=[Yc_m(id_cluster)-max(Yc_m) Yc_m(id_cluster)];
    zc=[-Zc(:,id_cluster)+x2(2) Zc(:,id_cluster)];
    fgc=ones(max(Lsub),Ntrans/2).*max(max(Yc));
    fgc(isnan(Yc(:,id_cluster)))=NaN;
    Yc=[Yc(:,id_cluster)-fgc Yc(:,id_cluster)];
    Ac=[Ac(:,id_cluster) Ac(:,id_cluster)];
    for iik=1:size(id_cluster,2)
        Cluster_ok{iik}(:,1)=Cluster{id_cluster(iik)}(:,1)-ones(size(Cluster{id_cluster(iik)},1),1)*max(max(NN));
        Cluster_ok{iik}(:,2)=Cluster{id_cluster(iik)}(:,2)+1;
    end
    for ilk=1:size(id_cluster,2)
        Cluster_ok{ilk+Ntrans/2}=Cluster{id_cluster(ilk)};
    end
    for kk=1:size(Cluster_ok,2)
        Lsub(kk)=size(Cluster_ok{kk},1);
    end

elseif flip_half_pannel==2
    %%%%%/%%%%
    Zc_m=[-Zc_m(id_cluster)+x2(2) Zc_m(id_cluster)];
    Yc_m=[Yc_m(id_cluster)-max(Yc_m) Yc_m(id_cluster)];
    zc=[-Zc(:,id_cluster)+x2(2) Zc(:,id_cluster)];
    fgc=ones(max(Lsub),size(o3,2))*ok1(1);
    provao1=ones(max(Lsub),size(o3,2))*ok1(1);
    provao3=ones(max(Lsub),size(o1,2))*ok3(1);
    for ik=1:size(o3,2)
        provao1(isnan(Yc(:,o3(ik))),ik)=NaN;
    end
    for ik=1:size(o1,2)
        provao3(isnan(Yc(:,o1(ik))),ik)=NaN;
    end
    for ik=1:size(o1,2)
        Yc(:,o1(ik))=provao3(:,ik);
    end
